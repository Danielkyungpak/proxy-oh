(function () {
    ...
    PokemonController.$inject = ['$scope', "$cardService", "$modalService"];

    function PokemonController($scope, $cardService, $modalService) {
        var vm = this;
        vm.$scope = $scope;
        ...
        function initialize() {
            ...
            if (localQueue) {
                ...
                console.log(vm.queue, "queue got")
            }
        ...
        async function _createPDF(horizontal) {
            vm.loading = true;

            var cardWidth = 2.45;
            var cardHeight = 3.4375;
            ...
            for (var a = 0; a < vm.queue.length; a++) {
                var promise = _getDataUri(vm.queue[a].imageUrlHiRes).then((imgData) => {
